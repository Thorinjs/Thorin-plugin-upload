...
module.exports = function init(thorin, opt, storageInstances) {
  ...
  function checkCors(res, handlerObj) {
    if(!handlerObj.cors) return;
    let corsDomain = handlerObj.cors.domain || '*';
    ...
    function handleIncomingRequest(req, res, next) {
      ...
      function attachBusboy(fileProcessFn, onDone) {
        ...
        function handleError(code, message) {
          ...
          isDone = true;
          remove();
          var err = (typeof code === 'string' ? thorin.error(code, message) : thorin.error(code));
          return onDone(err);
        }
          ...
          handlerObj.storageFn(intentObj, fileObj, (err, storageObj) => {
            ...
            try {
              fileProcessFn(intentObj, fileObj, (err) => {
                if(err) return handleError(err);
                if(isDone) return;
                storageObj.save(fileObj).then((uploadData) => {
                  if(isDone) return;
                  isDone = true;
