...
module.exports = function (thorin, opt) {
  ...
  class ThorinUploadHandler extends Action {
    ...
    _runCustomType(intentObj, handler, done) {
      if (handler.type === Action.HANDLER_TYPE.UPLOAD_PROCESS) {
        ...
        this._runHandler('before', 'upload.request', intentObj, null);
        if (intentObj.hasError()) {
          return done(intentObj.error());
        }
